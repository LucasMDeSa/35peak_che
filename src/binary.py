...
def unitless_coalescence_time(m, a, q):
    m = fix_unit(m, u.Msun)
    a = fix_unit(a, u.Rsun)
...
def unitless_coalescence_time(m, a, q):
    m *= ct.M_sun.cgs.value
    a *= ct.R_sun.cgs.value
    c = 5 / 256 * ct.c.cgs.value**5 / ct.G.cgs.value**3
    mass_term = m * q * m * (q + 1) * m
    tc = c * a**4 / mass_term
    tc *= u.s.to(u.yr)
    return tc
...
class WindIntegrator:
    ...
    def __init__(self, model_path, q0=1) -> None:
        ...
        self.mdot = -(10.0**self.h.log_abs_mdot)
...
class SV20WindIntegrator:
    ...
    def __init__(
        self, tau_ms, m0, a0, q0=1, dm=1.0, teff=10.0**4.75, z=Z_SUN, x=0.7
    ) -> None:
        self.tau_ms = tau_ms
        self.m0 = m0
        self.a0 = a0
