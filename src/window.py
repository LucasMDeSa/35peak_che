...
def mk_che_window_plot_arrs(m_min, m_max, res, test_q, test_z, wind_model, zams_mrr):
    ...
    print(
        "\n".join(
            (
                f"Computing constraints for {res} masses between {m_min} and {m_max}, "
    ...
    for i, p in enumerate(ppisne_test_ps):
        ...
        def f_to_min(m):
            ...
            m_tams, a_tams, p_tams, q_tams = get_tams(
                m, a_zams=a, q_zams=test_q, mixed_wind_model_dict=wind_model
            )
...
def mk_che_window_plot(ax, che_window_settings, constraint_arrays):
    ...
    ax.plot(
        test_masses, kepler_test_ps, "--", color="maroon", label="Critical rot.", lw=2
    )
    ...
    ax.fill_between(
        test_masses, t_gw_test_ps, np.tile(10, test_masses.shape), color="k", alpha=0.3
    )
    ax.fill_between(
        test_masses, np.tile(0.1, test_masses.shape), rlof_test_ps, color="k", alpha=0.3
    )
    ax.fill_between(
        test_masses, wind_test_ps, np.tile(10, test_masses.shape), color="k", alpha=0.3
    )

    ax.text(
        0.9,
        0.9,
        f"$\mathrm{{Z}}_\odot/{Z_SUN/test_z:.0f}$",
        ha="right",
        va="top",
