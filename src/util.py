...
def build_bump35_cmap(vmin=10.0, vmax=100.0, vcenter=35.0, base_cmap=cm.seismic):
    mpeak_norm = TwoSlopeNorm(vmin=vmin, vmax=vmax, vcenter=vcenter)
    bump35_cmap = LinearSegmentedColormap.from_list(
        "mpeak", base_cmap(mpeak_norm(np.linspace(vmin, vmax, 10000))), N=10000
    )
    return bump35_cmap


def build_logbump35_cmap(vmin=1.0, vmax=2.0, vcenter=1.54, base_cmap=cm.seismic):
    logmpeak_norm = TwoSlopeNorm(vmin=vmin, vmax=vmax, vcenter=vcenter)
    logbump35_cmap = LinearSegmentedColormap.from_list(
        "mpeak", base_cmap(logmpeak_norm(np.linspace(vmin, vmax, 10000))), N=10000
    )
...
def hd_limit(log_teff):
    ...
    log_l = (
        np.log10(1.0e5 * np.sqrt(4 * np.pi))
        + 0.5 * np.log10(ct.sigma_sb * u.Rsun**2 * teff**4 / u.Lsun)
    ).value
    log_l = max(log_l, np.log10(6.0e5))
    return log_l
...
def hr_diagram_tracks_base(
    ...
    r_to_plot = np.concatenate(
        (np.arange(0.5, 1, 0.1), np.arange(1, 10, 1), np.arange(10, 40, 10))
    )
    for i, r in enumerate(r_to_plot):
        ...
        if not i % 2:
            rotation = (
                -np.arctan((r_y[10] - r_y[0]) / (hd_x[10] - hd_x[0])) / np.pi * 180
            )
