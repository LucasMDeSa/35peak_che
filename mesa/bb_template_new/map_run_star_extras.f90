...
contains
  ...
  subroutine data_for_extra_history_columns(id, n, names, vals, ierr)
    ...
    ! wind regime information
    names(5) = 'he_poor_thick_wind'
    vals(5) = s% xtra(4) ! 0 or 1
    names(6) = 'he_poor_wind_weight'
    vals(6) = s% xtra(5) ! continuous
  ...
  integer function extras_finish_step(id)
    ...
    if ((s% lxtra(3) .eqv. .true.) .and. &
       ...
       write(*, '(/,a,/, 99e20.10)') &
                'stop because o depl', &
                'o16=', s% xa(s% net_iso(io16), s% nz), &
                'c12=', s% xa(s% net_iso(ic12), s% nz), &
                'he4=', s% xa(s% net_iso(ihe4), s% nz), &
                'h1= ', s% xa(s% net_iso(ih1), s% nz)
    ...
    ! Map grid: terminate when surface He4 first reaches 0.7
    if (s% xa(s% net_iso(ihe4), 1) >= 0.7d0) then
      write(*, '(/,a,/, 99e20.10)') &
               'stop because surface he4 >= 0.7', &
               s% xa(s% net_iso(ihe4), 1), 0.7d0
      extras_finish_step = terminate
    end if
