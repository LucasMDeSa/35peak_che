...
def get_system_flags(model_path):
    ...
    try:
        ...
        if prof.model_number <= h.model_number[-1]:
            # PATH A: C_depl profile is current → completed CHE model.
            # is_crit=F, is_l2of=F: model survived ZAMS → passed MESA's
            # omega>=omega_crit check → R < R_crit > R_L2 (for q=1 sync).
            # L2OF is a ZAMS-only condition (orbit widens from wind loss),
            # so surviving ZAMS guarantees no L2OF.
    ...
    if not is_C_depleted:
        try:
            ...
            # Interpretation: over-critical rotation → MESA terminated.
            # is_l2of=T: for q=1 sync, R_crit = 0.794a >> R_L2 = 0.500a,
            # so critical rotation strictly implies L2OF.
            ...
            if h.center_h1[-1] > ms_h1_threshold:
                # PATH B2: profile exists but center_h1 > 0.6 → barely evolved.
                # Same interpretation as B1: over-critical, MESA terminated.
                # is_l2of=T: R_crit >> R_L2 for q=1 sync (see B1).
                is_crit_at_zams = True
                is_l2of_at_zams = True
                is_merger_at_zams = True
                is_He_depleted = False
            ...
            # ── PATH B3: model evolved past MS (center_h1 ≤ 0.6) ──
            if not is_crit_at_zams:
                # Model survived MESA's omega>=omega_crit check at ZAMS,
                # so R < R_crit. But R could still exceed R_L2 (since
                # R_L2 = 0.500a < R_crit = 0.794a for q=1 sync), so
                # the geometry check is required.
                h = mr.MesaData(str(model_path / "LOGS/history.data"))
                zams_i = np.where(h.surf_avg_omega > 0)[0][0]
                r_zams = h.radius[zams_i]
                m_zams, p_zams = parse_folder_name(model_path)

                # B3 step 1: L2OF geometry check.
                ...
                if not is_l2of_at_zams:
                    # B3 step 2: depletion checks.
                    ...
                    # B3 step 3: CHE mixing check (only if not He-depleted).
