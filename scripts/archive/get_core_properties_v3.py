...
def read_system(model_path, z_key):
    ...
    if is_crit_at_zams or not is_He_depleted:
        ...
        for tahems_th in [1e-3, 1e-2, 1e-1]:
            try:
                tahems_i = np.where((h.center_h1 < 1e-3) & (h.center_he4 < tahems_th))[
                    0
                ][0]
            except IndexError:
                continue
...
def main():
    ...
    c = args.n_cores
    print(
        f"Starting collection with {c} cores, for model {model}, prefix {prefix}, with y_0={y_0} and delta_y={delta_y}"
    )
