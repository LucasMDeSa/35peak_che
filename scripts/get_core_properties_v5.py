    ...
    if not is_C_depleted:
        try:
            ...
            if not is_crit_at_zams:
                ...
                if (
                    final_center_h1 < 0.1
                    and final_center_he4 < 0.5
                    and final_center_c12 < 0.3
                ):
                    is_half_C_depleted = True
                    is_He_depleted = True
                    ...
                    is_half_C_depleted = False
                    if final_center_h1 < 1e-3 and final_center_he4 < 0.1:
                        is_He_depleted = True
...
def get_props(model_path, h, logs, stage_i, stage, prof_stage=None):
    ...
    conv_core_bound_i_stage = (
        len(prof_stage.mixing_type)
        - np.where(prof_stage.mixing_type[::-1] != 1)[0][0]
        - 1
    )
...
def try_to_get_profile(
    model_path, h, prof_fname, element_to_check, abundance_thresholds=[1e-3, 1e-2, 1e-1]
):
...
def main():
    ...
    parser.add_argument(
        "--n-cores", "-c", type=int, default=1, help="Number of cores to use"
    )
    parser.add_argument(
        "--model",
        "-m",
    ...
    parser.add_argument(
        "--y0", "-y", type=float, default=0.4, help="He-poor to -rich transition start"
    )
    parser.add_argument(
        "--delta-y",
        "-d",
    ...
    c = args.n_cores
    print(
        f"Starting collection with {c} cores, for model {model}, prefix {prefix}, with y_0={y_0} and delta_y={delta_y}, use_depletion_profiles={use_depletion_profiles}"
    )
